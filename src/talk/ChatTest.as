package talk {
	...
	public class ChatTest extends Sprite
	{
		...
		private function loginin(e:Event):void {
			...
			if (isTcp.selected) {
				...
				if (isLan.selected) {
					...
					// rtmfp://p2p.rtmfp.net/d7b76622ea79253a6bf3bd44-e1b437f463e4/
					server = new P2PConnecter("rtmfp://p2p.rtmfp.net/d7b76622ea79253a6bf3bd44-e1b437f463e4/");
				}
			}
...
}teMsg(talkPanel.input.text,CODE_TXT,talkPanel.isGroup);
			//				if (talkPanel.isGroup) {
			//					group.postMsg(msg);
			//				}
			//				
			//				talkPanel.input.text = "";
			//				talkPanel.receive(group.users,"you", msg.time, CODE_TXT, msg.data);
			//			}
		}
		...
		private function receive(group:P2PGroup, id:String, data:Object):void {
			var code:int=data.code
			
			//			var tp:TalkPanel = group2talkPanel[group];
			
			var e2:P2PUser = getUser(id);
			//			if (data.isGroup) {
			//				var tp2:TalkPanel = tp;
			//			}
			//			else {
			...
			//				}
			//			}
			talkPanel.receive(group.users,e2.name, data.time, code, data.data,e2);
			if (code == CODE_NAME) talkPanel.updateUserList(group.users.values());
		}
		
		private function getUser(id:String):P2PUser {
